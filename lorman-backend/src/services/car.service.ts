...
const CarritoService = {
    ...
    async createCarrito(data: Omit<CarritoDTO, "id_carrito">): Promise<CarritoDTO> {
        const existingCarrito = await CarritoRepository.findByCliente(data.id_cliente);
        if (existingCarrito) {
    ...
    async getCarritoByCliente(id_cliente: number): Promise<CarritoDTO> {
        const carrito = await CarritoRepository.findByCliente(id_cliente);
        if (!carrito) {
    ...
    async listCarritoDetails(id_carrito: number): Promise<CarDetailWithIdDTO[]> {
    ...
    async addProductToCarrito(data: CarDetailDTO): Promise<CarDetailWithIdDTO> {
        const carrito = await CarritoRepository.findByCliente(data.id_carrito);
        if (!carrito) {
